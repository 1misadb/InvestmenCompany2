    ...
    public class InvestmentDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Security>()
                .HasMany(s => s.АктивыПортфеля)
                .WithOne(a => a.ЦеннаяБумага)
                .HasForeignKey(a => a.ID_ЦеннойБумаги);
            modelBuilder.Entity<Portfolio>()
                .HasMany(p => p.АктивыПортфеля)
                .WithOne(a => a.Портфель)
                .HasForeignKey(a => a.ID_Портфеля);
            modelBuilder.Entity<SecuritySecurityType>()
                .HasOne(sst => sst.ЦеннаяБумага)
                .WithMany(s => s.ТипыЦенныхБумаг)
                .HasForeignKey(sst => sst.ID_ЦеннойБумаги);
            modelBuilder.Entity<SecuritySecurityType>()
                .HasOne(sst => sst.ТипЦеннойБумаги)
                .WithMany(st => st.ЦенныеБумаги)
                .HasForeignKey(sst => sst.ID_ТипаЦеннойБумаги);
            modelBuilder.Entity<Portfolio>()
                .HasMany(p => p.ТипыПортфелей)
                .WithOne(pt => pt.Портфель)
                .HasForeignKey(pt => pt.ID_Портфеля);
            modelBuilder.Entity<Security>()
                .HasMany(s => s.ИсторияКотировок)
                .WithOne()
                .HasForeignKey(h => h.ID_ЦеннойБумаги);
        }
    }
}